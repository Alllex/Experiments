...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if real_close(x1start,x1end)
      s1 = if y1start < y2start then self else seg end
      s2 = if y1start < y2start then seg else self end
      aXstart = s1.x1
      aYstart = s1.y1
      aXend = s1.x2
      aYend = s1.y2
      ...
      s1 = if x1start < x2start then self else seg end
      s2 = if x1start < x2start then seg else self end
      aXstart = s1.x1
      aYstart = s1.y1
      aXend = s1.x2
      aYend = s1.y2
